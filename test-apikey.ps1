...
Write-Host "$Blue=== QuickNotify API Key Testing ===$Reset"
Write-Host ""
...
if ($registerResponse.StatusCode -eq 201) {
    Write-Host "$Green[OK] Registration successful$Reset"
...
if ($generateKeyResponse.StatusCode -eq 201) {
    ...
    Write-Host "  Response: $($generateKeyResponse.Content)"
    exit 1
}
...
if ($keyInfoResponse.StatusCode -eq 200) {
    ...
    Write-Host "  Key Prefix: $($keyInfo.keyPrefix)"
    Write-Host ""
} else {
    Write-Host "$Red✗ Failed to fetch API Key info$Reset"
    Write-Host "  Response: $($keyInfoResponse.Content)"
    Write-Host ""
}
...
if ($getNotifResponse.StatusCode -eq 200) {
    ...
    Write-Host "  Total notifications: $($notifications.Length)"
    Write-Host ""
} else {
    Write-Host "$Red✗ Failed to retrieve notifications$Reset"
    Write-Host "  Response: $($getNotifResponse.Content)"
    Write-Host ""
}
...
if ($invalidKeyResponse.StatusCode -eq 401) {
    Write-Host "$Green✓ Correctly rejected invalid API Key$Reset"
    Write-Host ""
} else {
    Write-Host "$Red✗ Unexpected response for invalid API Key$Reset"
    Write-Host "  Status: $($invalidKeyResponse.StatusCode)"
    Write-Host ""
}
...
if ($revokeResponse.StatusCode -eq 200) {
    Write-Host "$Green✓ API Key revoked successfully$Reset"
    Write-Host ""
} else {
    Write-Host "$Red✗ Failed to revoke API Key$Reset"
    Write-Host "  Response: $($revokeResponse.Content)"
    Write-Host ""
}
...
if ($revokedKeyResponse.StatusCode -eq 401) {
    Write-Host "$Green✓ Correctly rejected revoked API Key$Reset"
} else {
    Write-Host "$Red✗ Revoked API Key still works (unexpected)$Reset"
    Write-Host "  Status: $($revokedKeyResponse.StatusCode)"
}
